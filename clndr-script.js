...
function populateEventModal(eventListing) {
    eventListing.forEach(function(currEvent) {
        ...
        if (currEvent.hasOwnProperty("cancelled") && currEvent.cancelled === true) {
            $("#eventText").append("<p class='text-center'><br/><h6 class='cancelled'>CANCELLED:<strike>" +
                currEvent.occasion + "</h6>invited: " +
                currEvent.invited_count + "</strike> </p>");
        } else {
            console.log(currEvent.date.toDate());
